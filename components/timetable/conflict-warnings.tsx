...
export function ConflictWarnings({ sessions }: ConflictWarningsProps) {
  ...
  for (const [key, count] of sessionMap) {
    if (count > 1) {
      ...
      if (session) {
        warnings.push(
          `Duplicate session: ${session.courseName} for ${session.groupName} with ${session.teacherName}`
        );
      }
    }
  }
  ...
  for (const [teacherId, count] of teacherSessions) {
    if (count > 8) {
      ...
      if (teacher) {
        warnings.push(
          `Teacher "${teacher.teacherName}" has ${count} sessions (may be overloaded)`
        );
      }
    }
  }
